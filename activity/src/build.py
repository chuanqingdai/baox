...
def main():
    ...
    MUST = {
        ...
        # ── 观测窗口（v1.4 新增）────────────────────────────────────────────
        # 窗口从「种子属性」升级为「用户可改的一等参数」，于是它的**入口**
        # 与它的**唯一口径实现**都要钉住。五条 id 逐条列出，不用计数式断言
        # （「有 5 个 win 开头的 id」这种计数命中 0 也算通过，本项目有先例）。
        '窗口预设组': 'class="win-presets"',
        '窗口起始日期框': 'id="winStart"',
        '窗口结束日期框': 'id="winEnd"',
        '窗口摘要只读': 'id="winInfo"',
        '窗口切分说明': 'id="winNote"',
        # 五个纯函数。少任何一个，抽屉里的日期框都会「能改但不生效」或
        # 「生效但不落盘」，而页面**不报错** —— 静默失效正是这里最需要门禁
        # 的理由（用户切窗口看到表格变短，只会怀疑自己点错了）。
        '窗口自然周切分': 'function naturalWeeks',
        '窗口归一化': 'function normalizeWindow',
        '窗口形状构造': 'function shapeWindow',
        '窗口当前值出口': 'function currentWindow',
        '周目标键格式迁移': 'function migrateWeekKeys',
    }
    # ── 必须用**词边界**判定的标记（铁律 16）────────────────────────────────
    ...
    MUST_RE = {
        ...
        # v1.4：窗口上下限判**赋值语句**。这两个常量是「抽屉能开多大的窗」
        # 的唯一来源，界面文案里的「1–90 天」是从它拼出来的 ——
        # 判子串时改回 365 也能被注释里的「90」满足（本文件恰好写着好几句
        # 讲 90 天的说明），那就成了「断言被注释接管」。
        '窗口上限常量': r"var\s+WIN_MAX_DAYS\s*=\s*90\s*;",
        '窗口下限常量': r"var\s+WIN_MIN_DAYS\s*=\s*1\s*;",
        # 周目标必须按**周起始日**存取。这条判的是**应用层那个 data 属性**，
        # 它有两个方向（正写在 MUST，反向写在本文件 MUST_NOT）——
        # 只写正向的话，谁把 app.js 改回 `w.idx` 而 core.js 仍按日期读，
        # 就会出现「周目标输入框能改、保存后套到别的周上」这种静默错位。
        '周目标按起始日写入': r'data-wkt="\' \+ w\.from \+ \'"',
    }
    # 每项必须有 / 必须无 双向断言
    MUST_NOT = {
        ...
        # ── v1.4 的反向断言：与上面「周目标按起始日写入」配对 ──────────────
        # 周序号只是**顺序**、不是身份：换窗口后同一个 W2 指向另一周，
        # 按序号存会让目标套到别的周上，达标率整体偏移，而且不报错。
        # 只写正断言不够 —— 两处写法可以同时存在（新增一处按日期、旧一处留序号），
        # 那时正向断言照样通过，而 UI 上会多出一个永远读不回来的输入框。
        '周目标按周序号写入残留': 'data-wkt="\' + w.idx + \'"',
    }
    # 「必须缺席」要在**去掉注释**的产物上判定：
    ...
    # ⚠️ v1.2e：只是「三种语法各剥一次」还不够 —— **要按语言分段剥**。
    ...
    # v1.4 追加说明：还有**第四处**消费者 —— 桌面导出件名（export-standalone.py）
    # 与它的截图脚本（shots-export.cjs）。它们不复制字面量，而是从 build.py
    # **读** VERSION，所以不需要第四条比对规则；写死的话，升版后截图脚本会去找
    # 一个已经不存在的老文件名，报出来的却是「找不到文件」这种无关的错。
    ...
    # ========================================================================
    # 门禁 A-6 · 抽屉分组顺序：观测窗口必须是第一组
    # ------------------------------------------------------------------------
    # 窗口是**数据的坐标系** —— 它决定下面每一组的日集合与周次切分。
    # 若它排在业务指标之后，用户改完日期往下滚，会发现自己刚填的表格
    # 已经换成另一段区间了，而滚动条不会告诉他这件事。
    # 顺序在这里是**设计约束**，不是排版偏好，所以判在源码里的出现顺序上。
    # 判据取「分组标题」这串唯一字面量（每组恰好一处）：不取 '观测窗口'
    # 这类短串 —— 它在注释与说明文案里还会出现好几次，判子串等于没判。
    # ========================================================================
    a6 = []
    grp = ['set-group-title">观测窗口', 'set-group-title">业务指标',
           'set-group-title">各周保费目标', 'set-group-title">逐日数据']
    pos = []
    for g in grp:
        n = parts['app.js'].count(g)
        if n != 1:
            a6.append('抽屉分组标题 %s 在 app.js 中出现 %d 次（应恰好 1 次）'
                      % (g.split('>')[-1], n))
        pos.append(parts['app.js'].find(g))
    if not a6 and pos != sorted(pos):
        a6.append('抽屉分组顺序不对：应为 观测窗口 → 业务指标 → 各周保费目标 → 逐日数据，'
                  '实为 %s' % ' → '.join(g.split('>')[-1] for g in grp))
    if a6 and not no_gate:
        print('!! 门禁 A-6 · 抽屉分组顺序失败:')
        for b in a6:
            print('     -', b)
        sys.exit(1)
    if not a6:
        print('门禁 A-6 · 抽屉分组顺序通过：观测窗口 → 业务指标 → 各周保费目标 → 逐日数据')
