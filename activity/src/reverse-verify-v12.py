...
无法证明任何单条判据不是多余的）：
  ...
  B  真代码里的对齐标记被删、**HTML 注释故意留着**（body.html 的 class + panel.css
     的规则都删，`<!-- g-self-top：… -->` 不动）→ 必须报「缺少 网格列表顶对齐类
     g-self-top」。B 与 E 是一对：同一种失效模式（**注释接管在场判定**），
     两种注释语法各一例 —— E 是 JS 块注释 `/* */`，B 是 HTML 注释 `<!-- -->`。
     v1.2d 之前只剥块注释，B 例构建**全绿**（假绿）；现在剥三种，B 例必须红。
  ...
  H  幂等重跑（连续两次构建）→ 必须全绿，且第 2 次的归档数量 == 第 1 次。
     判据刻意**不比沙箱初始值**：沙箱的 archive/ 是从本尊复制的，里面那份同名
     归档是本尊更早一次构建的内容，第 1 次构建合法地会分叉出一个新归档。
     拿初始值比会把这次必然分叉误判成「去重失效」（首版就这么写错过）。
...
def main():
    ...
    orig_css = rd(css)
    # 这一例刻意**只删真代码、保留注释**：
    #   <div class="panel g-self-top">        → <div class="panel">
    #   .g-self-top{align-self:start;}        → 删掉
    #   <!-- g-self-top：列表按自身内容收住 --> ← **留着**
    # 于是它与 E 例构成一对：同一种失效模式（注释接管在场判定），
    # 两种注释语法各一例 —— E 是 JS 块注释 /* */，B 是 HTML 注释 <!-- -->。
    # v1.2d 之前 probe_html 只剥块注释，B 例构建**全绿**（假绿）；
    # 现在剥三种，B 例必须红。这正是 B 例存在的理由：
    # 「去注释」少剥一种语法，就等于留一扇假绿的后门。
    case('B  真代码里的 g-self-top 被删（保留 HTML 注释）→ 必须红',
         True, '网格列表顶对齐类 g-self-top',
         lambda: (wr(body, orig_body.replace(
                     '<div class="panel g-self-top">', '<div class="panel">', 1)),
