...
十四个 case：
  ...
  J  身份键回退到旧名（identity.v1 → identity，**仍与业绩键不同**）
     → 必须只报「身份覆盖层键」，且**不得**报「身份键与业绩键相同」。
  K  两键同名（把 LS_ID 改成业绩键的值）→ 必须报「身份键与业绩键相同」。
     ⚠️ K **刻意破坏两个判据**（MUST_RE「身份覆盖层键」也会红，因为 LS_ID
     被改得不再等于被钉死的字面量）。这不是疏忽：
       · MUST_RE 把身份键钉在一个**精确字面量**上，所以任何对 LS_ID 的改动
         都必然先越过 MUST_RE —— 「只破坏分家一条」在物理上造不出来；
       · 于是一条反例证明不了分家判据。
     处置：用 **J/K 两态对照**替代单条反例 —— J 证明「两键不同时，分家分支
     一声不响」（它只报 MUST_RE），K 证明「两键相同时，多出那条分家消息」。
     多出来的这一条消息就是分家分支在干活，而不是死代码。
     诚实记一笔：分家判据在「MUST_RE 已把字面量钉死」的前提下**不是主检测器**，
     它的价值是**诊断**——它给出的是「恢复初始数据会连头像一起清掉」这个
     可执行的理由，而不是「缺少 身份覆盖层键（正则 …）」。
...
def main():
    ...
    case('A  旧 KPI 卡 id="kDays" 回来 → 必须红',
         True, '旧 KPI 打卡天数残留',
         lambda: mut(body, '<div class="stat-grid"',
                     '<div id="kDays">旧卡</div>\n<div class="stat-grid"'),
         restore_all)
    ...
    if have:
        dupname = os.path.join(sand_arch, 'activity-v1.9-260919-000000.html')
    ...
    # ---------- J 身份键回退到旧名（仍与业绩键不同）----------
    # 只破坏 MUST_RE「身份覆盖层键」：旧名与业绩键仍不相等，故分家判据不该响。
    case('J  身份键退回旧名(仍≠业绩键) → 必须只报「身份覆盖层键」',
         True, '缺少 身份覆盖层键',
         lambda: mut(core, "'baox.act.identity.v1'", "'baox.act.identity'"),
         restore_all, must_absent='身份键与业绩键相同')

    # ---------- K 两键同名 ----------
    # ⚠️ 刻意破坏两条（MUST_RE 也会红）：MUST_RE 把身份键钉在精确字面量上，
    #    「只破坏分家一条」造不出来。故 J/K 成对使用 ——
    #    J 证明两键不同时那条消息一声不响，K 证明两键相同时它多出来。
    case('K  两键同名 → 必须报「身份键与业绩键相同」（与 J 成对）',
         True, '身份键与业绩键相同',
         lambda: mut(core, "'baox.act.identity.v1'", KEY_OLD),
         restore_all)
